...
export function SongList({ musicManager }: SongListProps) {
  const onPlay = (item: QueueData) => {
    musicManager.queueManager.setIndex(item.id);
  };

  const onRemove = (item: QueueData) => {
    ...
    <div className="flex flex-col -mx-2 -mt-2">
      {musicManager.queueManager.songs.map((song) => (
        <Item
          key={song.id}
          song={song}
          playing={song.id === musicManager.queueManager.currentSong}
          onPlay={onPlay}
          onRemove={onRemove}
        />
      ))}
...
function Item({
  ...
  song: QueueData;
  playing: boolean;
  onPlay: (item: QueueData) => void;
  onRemove: (item: QueueData) => void;
}) {
  return (
    <button
      className={cn(
    ...
    >
      ...
      <div>
        ...
        <p className="text-xs text-purple-200">{song.artist}</p>
      </div>
      {song.isCustom && (
        <div
